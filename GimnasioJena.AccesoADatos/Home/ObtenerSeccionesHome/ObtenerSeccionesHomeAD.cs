...
namespace GimnasioJena.AccesoADatos.Home.ObtenerSeccionesHome
{
    public class ObtenerContenidoWebAD : IObtenerContenidoWebAD
    {
        public async Task<IEnumerable<ContenidoWebDto>> ObtenerPorPaginaAsync(string pagina)
        {
            using (var contexto = new Contexto())
            {
                ...
                if (tablaVacia)
                    ...
                    await ParchearRegistrosFaltantes(contexto, pagina);
        ...
        private async Task ParchearRegistrosFaltantes(Contexto contexto, string pagina)
        {
            if (pagina != "Home")
                return;
            ...
            foreach (var esperado in registrosEsperados)
            {
                bool existe = await contexto.ContenidoWeb
                    .AnyAsync(x => x.Pagina == "Home" && x.Clave == esperado.Clave);
